...
- SCOPED SALARY EXCEPTION: the ONLY exception to the never-invent rule is the exact question "What are your salary expectations?", which may give one broad, approximate resume-based market range (see PERSONAL-PREFERENCE QUESTIONS). Even there, you may only supply the estimated range and compensation type - never an invented candidate history, current/desired salary, employer salary band, role, location, skill, qualification, or achievement.
...
- Each question normally includes one spoken sample answer:
  ...
  - First person, direct, and conversational; use contractions. Do not sound like a formal cover letter, and avoid exaggerated or promotional language.
...
- Salary ("What are your salary expectations?"): act as a senior recruiter, compensation specialist, and expert English writer. Using ONLY the resume, write a confident, polished, natural and grammatically correct spoken answer of EXACTLY TWO sentences and 35 to 50 words (this overrides the general sample length for this question only). First silently identify the candidate's likely target role, location, industry, career level, relevant experience, strongest qualifications, and the appropriate compensation type; then estimate ONE broad, rounded range in the local currency as informed resume-based guidance.
  - Write in the FIRST PERSON throughout: say "my", never "your", when referring to the candidate.
  - Sentence 1: open with the candidate's strongest resume-supported reason for the range, include one or two strong resume facts, state the rounded range, and name the correct pay type (annual base salary, hourly base pay, monthly, contract/daily rate, or base plus OTE).
  ...
  - Use SPECIFIC resume evidence - relevant experience, leadership, technical projects, clinical background, budget ownership, certifications, or measurable responsibility. Do NOT use vague filler such as "my eagerness to contribute", "my strong academic background", "I am hardworking", or "I am passionate". Mention no more than two evidence themes and no more than three individual skills.
  - VARY the OPENING to fit the candidate's strongest evidence - do NOT start every answer the same way, do NOT automatically begin with "Considering my", and do NOT open with a list of skills. Use ONE natural opener such as: "Based on my [N] years of...", "With my background in...", "Given my experience leading...", "Drawing on my experience...", "With my degree and hands-on experience...", or "I'm targeting...".
  - Choose the target role in priority order: explicit desired title; else the target stated in the professional summary; else the current or most-recent role; else the dominant recent career path. Choose the location from the resume (stated target, else current, else most-recent job, else country only). Never invent a role or location.
  - NO reliable location/currency: if no reliable location, country, or currency can be established from the resume, do NOT invent one and do NOT give a number - instead use the safe, non-committal answer that asks for the employer's budgeted range first (e.g. "I'd want to understand the budgeted range for the role first; I'm confident we can find a fair number given my experience."), and make one coaching tip remind the candidate to research their own target range. Only estimate a range (below) when a location/currency IS available; widen it when the role or seniority is unclear.
  - Use natural, negotiation-friendly figures (e.g. "$80K to $95K annually", "$35 to $42 an hour", or "$90K base with $140K to $160K OTE"), never overly precise numbers like $82,417. Do NOT confuse base salary with bonus, commission, equity, benefits, shift differentials, total compensation, or OTE.
  - Widen the range when the role, location, career level, or relevant experience is unclear: clear role/location/experience about 15-25%; career-changer or partial match about 20-35%; missing location or unclear role about 25-40%; sales, executive or contract roles wider still.
  - Close with flexibility, and VARY the wording (do not reuse one line): e.g. "I'm flexible depending on the role's scope and overall compensation package" / "I'm open to discussing where my experience fits within the employer's range" / "There's flexibility depending on the responsibilities, benefits, and complete package".
  - Present it as informed, resume-based guidance. Never claim it comes from live market data, a salary survey, an employer database, a job posting, or current market research. Never invent a role, location, dates, experience, skills, qualifications, achievements, current salary, desired salary, or an employer salary band, and never claim the candidate already earns or demands a specific figure.
  - Do NOT use awkward or canned phrases such as: "I'm after the market rate", "my background leads me to expect", "the role matters more than salary", "full compensation picture", "according to current market data", "anything is fine", "salary does not matter", "this is the minimum I will accept", or "I know I'm worth".
  - Final check before returning: exactly two sentences and 35 to 50 words; "my" not "your"; the pay type suits the profession; every candidate-specific detail appears in the resume; the wording is grammatically correct and natural. Return only the final spoken answer - no headings, bullets, reasoning, confidence labels, or extra commentary.
...
experience: """${(resume.experience || "").slice(0, 2500)}"""`;
        ...
        if (type === "screening") {
          ...
          const SALARY_STYLES = [
            "Experience-first: open with the most relevant experience ('Based on my N years of...'), THEN the range and pay type.",
            "Background-first: open with the candidate's field or background ('With my background in...'), THEN the range and pay type.",
            "Leadership-first: open with leadership or the scope owned ('Given my experience leading...'), THEN the range and pay type.",
            "Experience-drawn: open with 'Drawing on my experience...', THEN the range and pay type.",
            "Qualification-first: open with education/certifications/projects ('With my degree and hands-on experience...'), THEN the range and pay type.",
            "Direct-range-first: open by stating the target directly ('I'm targeting...').",
          ];
          const salaryStyle =
          ...
          const SALARY_CLOSINGS = [
            "I'm flexible depending on the role's scope and overall compensation package.",
            "I'm open to discussing where my experience fits within the employer's range.",
            "There's flexibility depending on the responsibilities, benefits, and complete package.",
            "I'm flexible depending on the position's scope and the complete package.",
          ];
          const salaryClosing =
